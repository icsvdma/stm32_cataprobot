...
|--------|--------|------|---------|---------------|
| `0x01` | `CMD_MOTOR_SPEED` | DC モーター速度設定 | 後述 | **実装済み** (TB6612) |
| `0x02` | `CMD_MOTOR_STOP` | DC モーター停止 | 後述 | **実装済み** (TB6612) |
| `0x03` | `CMD_STEPPER_CTRL` | ステッピングモーター制御 | (未定義) | **暫定実装**: LED3 を点灯する |
| `0x04` | `CMD_EMERGENCY_STOP` | 緊急停止 | (なし) | **実装済み** |
| `0x10` | `CMD_STATUS_REQ` | ステータス要求 | (未定義) | 未実装 (weak stub) |
| `0x11` | `CMD_LED_MODE` | ヘッドライト LED モード設定 | 後述 | **実装済み** |
| `0x12` | `CMD_LED_BRIGHTNESS` | ヘッドライト LED 輝度設定 | 後述 | **実装済み** |
...
TB6612FNG デュアル H ブリッジモータードライバを SPI 経由で制御するコマンド群。  
PWMA (PC0/TIM1_CH1) と PWMB (PC1/TIM1_CH2) で 20kHz PWM 速度制御、  
PC6/PC7 (AIN1/AIN2)、PC8/PC9 (BIN1/BIN2) で方向制御、PC2 (STBY) でスタンバイ制御を行う。

### CMD_MOTOR_SPEED (0x01) — モーター速度設定

A/B チャンネルの回転方向と速度を設定する。

#### Payload 構造

| Byte | フィールド | 説明 |
|------|-----------|------|
| `payload[0]` | `ch` | 対象チャンネル |
| `payload[1]` | `dir` | 回転方向 |
| `payload[2]` | `speed_H` | 速度 上位バイト (0〜1000) |
| `payload[3]` | `speed_L` | 速度 下位バイト |
...
| 値 | 定義名 | 動作 | xIN1 | xIN2 |
|----|--------|------|------|------|
| `0x00` | `TB6612_DIR_CW` | 正転 | HIGH | LOW |
| `0x01` | `TB6612_DIR_CCW` | 逆転 | LOW | HIGH |
| `0x02` | `TB6612_DIR_BRAKE` | ブレーキ（ショートブレーキ） | HIGH | HIGH |
| `0x03` | `TB6612_DIR_COAST` | コースト（慣性回転） | LOW | LOW |

#### speed 値

- 範囲: 0〜1000（PWM duty に比例）
- 0 = 停止、1000 = 最大速度
- 急激な変化はスルーレート制限により緩和される

#### 送信例

A チャンネルを正転 50% (speed=500) で回転:
```
0xAA 0x01 0x04 0x00 0x00 0x01 0xF4 [checksum]
```
- `0xAA` = Header
- `0x01` = CMD_MOTOR_SPEED
- `0x04` = Length (payload 4 bytes)
- `0x00` = ch (A チャンネル)
- `0x00` = dir (CW 正転)
- `0x01` = speed_H (500 >> 8 = 1)
- `0xF4` = speed_L (500 & 0xFF = 244)
- `checksum` = `0x01 ^ 0x04 ^ 0x00 ^ 0x00 ^ 0x01 ^ 0xF4` = `0xF0`

両チャンネルを逆転 100% (speed=1000) で回転:
```
0xAA 0x01 0x04 0xFF 0x01 0x03 0xE8 [checksum]
```
- `0xFF` = ch (両方)
- `0x01` = dir (CCW 逆転)
- `0x03` `0xE8` = speed (1000)
- `checksum` = `0x01 ^ 0x04 ^ 0xFF ^ 0x01 ^ 0x03 ^ 0xE8` = `0x10`

### CMD_MOTOR_STOP (0x02) — モーター停止
...
| Byte | フィールド | 説明 |
|------|-----------|------|
| `payload[0]` | `ch` | 対象チャンネル（0x00=A, 0x01=B, 0xFF=両方）。省略時は全チャンネル停止 |

#### 送信例

B チャンネルのみ停止:
```
0xAA 0x02 0x01 0x01 [checksum]
```
- `0x02` = CMD_MOTOR_STOP
- `0x01` = Length (payload 1 byte)
- `0x01` = ch (B チャンネル)
- `checksum` = `0x02 ^ 0x01 ^ 0x01` = `0x02`

全チャンネル停止（payload 省略）:
```
0xAA 0x02 0x00 [checksum]
```
- `0x00` = Length (payload なし)
- `checksum` = `0x02 ^ 0x00` = `0x02`

### CMD_EMERGENCY_STOP (0x04) — 緊急停止

全モーターを即座にブレーキ停止し、STBY を LOW にする。  
Payload は不要。スルーレート制限を無視して即時停止する。
...
- `0x00` = Length (payload なし)
- `checksum` = `0x04 ^ 0x00` = `0x04`

> **注意**: 緊急停止後にモーターを再動作させるには、ESP32 側で再起動シーケンスを実行すること。

### 反転シーケンス

回転中に逆方向を指令された場合、以下の安全シーケンスが自動実行される:

```
1. スルーレート制限で現在速度を 0 まで減速
2. 100ms の停止待機
3. 新方向にピン切替
4. スルーレート制限で目標速度まで加速
```
...
| PWM 周波数 | 20kHz (TIM1, PSC=0, ARR=3199) |
| PWMA | PC0 (TIM1_CH1) |
| PWMB | PC1 (TIM1_CH2) |
| STBY | PC2 (GPIO Output) |
| AIN1 / AIN2 | PC6 / PC7 (GPIO Output) |
| BIN1 / BIN2 | PC8 / PC9 (GPIO Output) |
